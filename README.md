# RSM Pulse — Active Directory Security Scanner
...
git clone https://github.com/rsmusllp/RSM-Pulse.git
cd rsm-pulse
...
```bash
python rsm-pulse.py --domain corp.local --user jsmith --password 'P@ssw0rd!'
```
...
```bash
python rsm-pulse.py --domain corp.local --user jsmith --password 'P@ssw0rd!' --dc-ip 10.0.0.1
```
...
```bash
# Console only
python rsm-pulse.py --domain corp.local --user jsmith --password 'P@ssw0rd!' --report console

# JSON only
python rsm-pulse.py --domain corp.local --user jsmith --password 'P@ssw0rd!' --report json

# HTML only
python rsm-pulse.py --domain corp.local --user jsmith --password 'P@ssw0rd!' --report html

# All formats (default)
python rsm-pulse.py --domain corp.local --user jsmith --password 'P@ssw0rd!' --report all
```

### Custom Output Directory

```bash
python rsm-pulse.py --domain corp.local --user jsmith --password 'P@ssw0rd!' --output-dir /tmp/scans
```
...
```bash
python rsm-pulse.py --domain corp.local --user jsmith --password 'P@ssw0rd!' --no-color
```
...
```
rsm-pulse/
├── rsm-pulse.py          # Entry point — argument parsing, orchestration
...
1. **Connect** — RSM Pulse binds to the target DC over LDAPS (port 636) with automatic fallback to LDAP (port 389). Authentication is attempted via NTLM and SIMPLE bind.
...
All operations are **read-only**. RSM Pulse does not modify any AD objects, group memberships, GPOs, or ACLs.

---

## Security Considerations

- RSM Pulse requires valid domain credentials. Store and transmit credentials securely.
...
- Run RSM Pulse from a trusted, hardened workstation on the target network.

---

## Limitations

- **Registry-only settings** — NTLMv1 (`LmCompatibilityLevel`), WDigest (`UseLogonCredential`), LDAP signing (`ldapServerIntegrity`), and channel binding (`ldapEnforceChannelBinding`) cannot be read via LDAP. RSM Pulse flags these as manual verification items.
- **GPO content** — RSM Pulse checks GPO metadata (flags, version, SYSVOL path, links) but does not parse GPO settings files from SYSVOL (with the exception of the cpassword scan in check 25).
...
RSM Pulse is provided as-is for **authorised security assessments only**. Always obtain written permission before scanning any Active Directory environment. The authors are not responsible for misuse or any damage caused by this tool.
